...
print("="*60)
print(f"DISPOSITIVO ATTIVO PER IL TEST 8X BREATHPOOL: {device.type.upper()}")
if device.type == "cuda":
...
# BreathPool8x FFN
class BreathPool8xFFN(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        self.input_proj = nn.Linear(d_model, 8 * d_model)
        self.act = nn.GELU()
        self.pool = nn.AdaptiveMaxPool1d(d_model)
        self.norm = nn.LayerNorm(d_model)
        
    def forward(self, x):
        B, T, C = x.size()
        x_flat = x.view(B * T, C)
        x_expanded = self.act(self.input_proj(x_flat))
        
        # x_expanded shape: [B*T, 8*C]
        x_unsqueezed = x_expanded.unsqueeze(1) # [B*T, 1, 8*C]
        pooled = self.pool(x_unsqueezed).squeeze(1) # [B*T, C]
        
        out_flat = self.norm(pooled)
...
class Block(nn.Module):
    def __init__(self, d_model, n_head, block_size):
        ...
        self.ffn = BreathPool8xFFN(d_model)
...
class GPT(nn.Module):
    ...
    def generate(self, idx, max_new_tokens):
        for _ in range(max_new_tokens):
            idx_cond = idx[:, -self.block_size:] if hasattr(self, 'block_size') else idx[:, -128:]
...
def train_gpt(seed):
    print("\n" + "="*70)
    print(f" ADDESTRAMENTO GPT BPE 8X BREATHPOOL | SEED: {seed}")
    print("="*70)
...
if __name__ == '__main__':
    ...
    df_curves = pd.DataFrame({
        "Step": steps,
        "eightx_Train": [x[1] for x in l],
        "eightx_Val": [x[2] for x in l]
    })
    df_curves.to_csv("loss_curves_bpe_8x.csv", index=False)
    print("\n-> Curve di loss salvate con successo in 'loss_curves_bpe_8x.csv'")
    
    # Save Summary Report
    summary = [{
        "Config": "breath_pool_8x",
        "Params": p,
        "Time": t,
    ...
    df_sum.to_csv("transformer_bpe_8x_results.csv", index=False)
    
    # Print Report
    print("\n" + "="*90)
    print(f"     REPORT NANO-GPT BPE 8X BREATHPOOL | SEED: {seed}")
    print("="*90)
    ...
    print(f"\n" + "-"*35 + f" TESTO GENERATO DA GPT 8X BREATHPOOL " + "-"*35)
    print(txt)
    print("-"*100)
