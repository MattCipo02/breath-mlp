...
print("="*60)
print(f"DISPOSITIVO ATTIVO PER IL TRANSFORMER BPE (CLEAN): {device.type.upper()}")
if device.type == "cuda":
...
# BreathMLP uses start_width = FFN_START_MULT * d_model
...
def train_gpt(ffn_type, seed):
    print("\n" + "="*70)
    print(f" ADDESTRAMENTO GPT BPE (CLEAN): {ffn_type.upper()} | SEED: {seed}")
    print("="*70)
...
if __name__ == '__main__':
    ...
    df_curves.to_csv("loss_curves_bpe_clean.csv", index=False)
    print("\n-> Curve di loss salvate con successo in 'loss_curves_bpe_clean.csv'")
    ...
    df_sum.to_csv("transformer_bpe_clean_results.csv", index=False)
    
    # Print Report
    print("\n" + "="*90)
    print(f"     REPORT COMPARATIVO NANO-GPT BPE (CLEAN) | SEED: {seed}")
    print("="*90)
    ...
    for cfg in configs:
        print(f"\n" + "-"*35 + f" TESTO GENERATO DA GPT {cfg.upper()} " + "-"*35)
        print(results[cfg]["text"])
        print("-"*100)
