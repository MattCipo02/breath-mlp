...
print("="*60)
print(f"DISPOSITIVO ATTIVO PER IL TEST ViT CON INTERNAL SKIP: {device.type.upper()}")
if device.type == "cuda":
...
# Breath FFN (Linear) with Internal Skip Connection
class BreathFFNSkip(nn.Module):
    ...
    def forward(self, x):
        B, T, C = x.size()
        x_flat = x.view(B * T, C)
        # Skip connection: net(x_flat) + x_flat
        out_flat = self.net(x_flat) + x_flat
        return out_flat.view(B, T, C)

# BreathPool Max FFN with Internal Skip Connection
class BreathPoolMaxFFNSkip(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        hidden_layers = [4 * d_model] # [768]
...
class ViTBlock(nn.Module):
    def __init__(self, d_model, n_head, ffn_type):
        ...
        if ffn_type == "breath_skip":
            self.ffn = BreathFFNSkip(d_model)
        elif ffn_type == "breath_pool_max_skip":
            self.ffn = BreathPoolMaxFFNSkip(d_model)
        else:
...
class SimpleViT(nn.Module):
    def __init__(self, num_classes=10, ffn_type="breath_skip"):
        super().__init__()
...
if __name__ == '__main__':
    ...
    configs = ["breath_skip", "breath_pool_max_skip"]
    ...
    df_raw.to_csv("vit_internal_skip_raw_results.csv", index=False)
    ...
    print("\n" + "="*90)
    print("             REPORT COMPARATIVO INTERNAL SKIP ViT (CIFAR-10)")
    print("="*90)
