...
from breath_mlp import BreathMLP

# Configuration
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("="*60)
print(f"DISPOSITIVO ATTIVO PER IL TRANSFORMER: {device.type.upper()}")
if device.type == "cuda":
...
class BreathFFN(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        # Standard FFN has ~295k parameters
        # Breath sequence is scaled to be close (~268k parameters)
        hidden_layers = [576, 144, 288, 36, 72]
        self.net = BreathMLP(
            input_dim=d_model,
    ...
    def forward(self, x):
        # We need to flatten the batch/sequence dimensions for BreathMLP,
...
def train_gpt(ffn_type):
    print("\n" + "="*70)
    print(f" ADDESTRAMENTO GPT CON FFN TYPE: {ffn_type.upper()}")
    print("="*70)
...
print("\n" + "="*80)
print("             REPORT COMPARATIVO NANO-GPT (TINY SHAKESPEARE)")
print("="*80)
